...
function writeLog(str) {
    let file;
    if (fs.existsSync(sysVariable.userData + "\\calculator") && fs.statSync(sysVariable.userData + "\\calculator").isFile()) {
        file = JSON.parse(atob(fs.readFileSync(sysVariable.userData + "\\calculator", "utf-8")));
    } else {
        file = [];
    }
    file.unshift(str);
    fs.writeFileSync(sysVariable.userData + "\\calculator", btoa(JSON.stringify(file)));
    readLog();
}
