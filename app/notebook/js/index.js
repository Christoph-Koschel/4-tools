...
function loadNoteEntries() {
    const path = sysVariable.userData + "\\note";
    let entries = ""
    if (fs.existsSync(path) && fs.statSync(path).isFile()) {
        entries = JSON.parse(atob(fs.readFileSync(path, "utf8")));
    } else {
        entries = [];
    }
...
function saveNotes() {
    const path = sysVariable.userData + "\\note";
    fs.writeFileSync(path, btoa(JSON.stringify(activeEntries)));
}
...
const emoji = {
    size: 15,
    icons: [],
    iconsPath: __dirname + "\\emoji\\emoji.json",
