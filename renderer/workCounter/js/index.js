...
window.addEventListener("load", () => {
    ...
    function loadEntries() {
        ...
        if (fs.existsSync(setting.counter.fileList) && fs.statSync(setting.counter.fileList).isFile()) {
            ...
            for (let i = 0; i < list.length; i++) {
                table.innerHTML += `<tr><td><a>${new Date(list[i].start).getDate() + "." + (new Date(list[i].start).getMonth() + 1) + "." + new Date(list[i].start).getFullYear() + "&nbsp;" + new Date(list[i].start).getHours() + ":" + new Date(list[i].start).getMinutes() + ":" + new Date(list[i].start).getSeconds()}</a></td><td><a>${new Date(list[i].stop).getDate() + "." + (new Date(list[i].stop).getMonth() + 1) + "." + new Date(list[i].stop).getFullYear() + "&nbsp;" + new Date(list[i].stop).getHours() + ":" + new Date(list[i].stop).getMinutes() + ":" + new Date(list[i].stop).getSeconds()}</a></td><td><ul><li>${"Milliseconds: " + (new Date(list[i].stop - list[i].start).getMilliseconds() - 1)}</li><li>${"Seconds: " + new Date(list[i].stop - list[i].start).getSeconds()}</li><li>${"Minutes: " + new Date(list[i].stop - list[i].start).getMinutes()}</li><li>${"Hours: " + (new Date(list[i].stop - list[i].start).getHours() - 1)}</li></ul></td></tr>`;
            }
        }
